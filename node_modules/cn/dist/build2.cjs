...
const themeFromCss = (entries) => {
	...
	const visit = (path) => {
		...
		let css;
		try {
			css = stripComments((0, node_fs.readFileSync)(path, "utf8"));
		} catch (err) {
			throw new Error(`cannot read css ${path === entry ? path : `${path} (imported from ${entry})`}: ${err.message}`, { cause: err });
		}
		for (const spec of importPaths(css)) visit((0, node_path.resolve)((0, node_path.dirname)(path), spec));
	...
	for (const file of entries) {
		entry = (0, node_path.resolve)(file);
		visit(entry);
	}
	const extend = {};
	const override = {};
...
const findTailwindCss = (cwd) => expandGlobs(CSS_GLOBS, cwd).filter((file) => {
	try {
		return isTailwindCss((0, node_fs.readFileSync)(file, "utf8"));
	} catch {
		return false;
	}
});
...
const build = async (options = {}) => {
	...
	if (options.css !== false) {
		const entries = options.css === void 0 ? findTailwindCss(cwd) : [(0, node_path.resolve)(cwd, options.css)];
		const theme = themeFromCss(entries);
		config = require_compiler.mergeConfigs(config, theme.extension);
...
const createBuilder = (options = {}) => {
	...
	const isThemeSource = (abs) => {
		...
		try {
			return isTailwindCss((0, node_fs.readFileSync)(abs, "utf8"));
		} catch {
			return false;
		}
	};
...
Object.defineProperty(exports, 'createContentMatcher', {
  enumerable: true,
  get: function () {
    return createContentMatcher;
  }
});
Object.defineProperty(exports, 'expandGlobs', {
  enumerable: true,
  get: function () {
    return expandGlobs;
  }
});
Object.defineProperty(exports, 'extractTokens', {
  enumerable: true,
  get: function () {
